...
public class UtilsBean implements Serializable {
    ...
    public NameMapBean[] formatHyperlinks(NameMapBean[] nameMaps, String inputType) {
        for (NameMapBean nameMap : nameMaps) {
            if (nameMap.getHit().equals("") && inputType.equals("name")) {
                nameMap.setQuery("<strong style=\"background-color:yellow; font-size=125%; color=\"black\">" + nameMap.getQuery() + "</strong>");
                nameMap.setDetails("View");
            } else if (nameMap.getHit().equals("")) {
                nameMap.setQuery("<strong style=\"background-color:red; font-size=125%; color=\"black\">" + nameMap.getQuery() + "</strong>");
            } else {
    ...
    public String performNameMapping() {
        ...
        // call API to perform name mapping
        String result = null;
        ...
        //If API call fails, switch to local code
        if (!useApiCalls) {
            RConnection RC = sb.getRConnection();
            try {
                RDataUtils.setMapData(RC, qVec);
                ...
                //e.printStackTrace();
                LOGGER.error("performSanityCheck-API", e);
                return null;
            }
        } else {
            return result;
    ...
    public void performLipidAnalysis() {
        ...
        RConnection RC = sb.getRConnection();
        String fileName = DataUtils.uploadFile(lipidFile, sb.getCurrentUser().getHomeDir(), null, ab.isOnPublicServer());
        try {
    ...
    public void writeMapTable(NameMapBean[] nameMaps) {
        ...
        try {
            FileWriter myWriter = new FileWriter(sb.getCurrentUser().getHomeDir() + "/name_map.csv");
            if (nameMaps2 != null) {
                myWriter.write("\"Query\"," + "\"Match\"," + "\"HMDB\"," + "\"PubChem\"," + "\"ChEBI\"," + "\"KEGG\"," + "\"METLIN\"," + "\"SMILES\"," + "\"Comment\"\n");
                for (int i = 0; i < nameMaps2.length; i++) {
                    myWriter.write(prepareNameMapLine(nameMaps2[i]));
                }
            }
            myWriter.close();
