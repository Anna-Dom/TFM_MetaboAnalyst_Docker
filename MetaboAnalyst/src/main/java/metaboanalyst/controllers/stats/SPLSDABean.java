...
public class SPLSDABean implements Serializable {
    ...
    public void setCvOpt(String cvOpt) {
        ...
        if (cvOpt.equals("loo")) {
            ...
            if (sb.getSampleNumber() > 60) {
                DataUtils.updateMsg("Warning", "Reset to 5-fold CV. LOOCV is only permitted for small data (< 60) samples on server.");
                validationOpt = "Mfold";
                this.cvOpt = "5";
            }
        } else {
    ...
    public void splsScore3dBtn_action() {
        if (compNum > 2) {
            ...
            DataUtils.updateMsg("Error", "The current model contains less than 3 components!");
        }
    }
