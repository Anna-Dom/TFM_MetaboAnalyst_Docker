...
//import Line2 from "https://cdn.skypack.dev/-/three-line2@v1.1.9-NWIpwt7MGkH02Hbal0oc/dist=es2019,mode=imports/optimized/three-line2.js";
import {
Object3D,
        LineSegments,
        EdgesGeometry,
...
from "https://cdnjs.cloudflare.com/ajax/libs/three.js/r132/three.module.js";
...
from './threejsmodules/BufferGeometryUtils.js';
import { OrbitControls }
from './threejsmodules/OrbitControls.js';
import SpriteText from './three-spritetext.module.js';
import {ConvexGeometry}
from './convexthree.module.js';
import Rainbow from 'https://cdn.skypack.dev/pin/@indot/rainbowvis@v1.0.2-lTmrzOpfDQmRcxIejuC9/mode=imports,min/optimized/@indot/rainbowvis.js';
...
function initScatter3D(jsonNm) {
    ...
    $.getJSON('/MetaboAnalyst' + jsonNm, function (raw_data) {
        ...
        if (Array.isArray(gData.meta)) {
            ...
            gData.meta = newObj;
            //var select = document.getElementById('metaopt');
            //console.log(select);
            //var opt = document.createElement('option');
            //opt.value = "Metadata";
            //opt.innerHTML = "Metadata";
            //select.appendChild(opt);
            //$('#metaopt').val("Metadata");
        }
...
function setupNetworkFunctions() {
    ...
    $('#bgColMainOpt').change(function () {
        var val = $(this).val();
        mainCheckBool = true;
        changeBackground(val, val);
    });

    $('#bgColInsetOpt').change(function () {
        var val = $(this).val();
        mainCheckBool = false;
        changeBackground(val, val);
        mainCheckBool = true;
...
function createText2D(text, idModifier, sceneId) {
...
function initGrids(scene, nodes, type, visible) {
    ...
    for (let i = 0; i <= ticksX.length; i = i + 2) {
        ...
        const xLabel = createText2D(ticksX[i] + "", "_xtick@_" + type, scene.uuid)
    ...
    for (let i = 0; i <= ticksY.length; i = i + 2) {
        ...
        const yLabel = createText2D(ticksY[i] + "", "_ytick@_" + type, scene.uuid)
    ...
    for (let i = 0; i <= ticksZ.length; i = i + 2) {
        ...
        if (ticksZ[i] !== undefined) {
            const zLabel = createText2D(ticksZ[i] + "", "_ztick@_" + type, scene.uuid)
